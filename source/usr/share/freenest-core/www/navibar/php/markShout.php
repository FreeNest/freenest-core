<?php
	...
	$sender = strip_tags($_POST['from']);
	$messageId = strip_tags($_POST['id']);
	$sender= str_replace(";","",$sender);
	$messageId = str_replace(";","",$messageId);
   
	markMsg($uid,$dbname,$uname,$pw, $sender, $messageId);

	echo "ok";

	function markMsg($uid, $dbname, $uname, $pw, $sender, $messageId){
		try{
	...
	//	$ldapUid=getLdapUid($recipient);
		mysql_query("UPDATE message SET unread=0 WHERE id=$messageId AND sender_uid=$sender");
      ...
      $query= "INSERT INTO $dbname.message(uid, message, unread, sender_uid) VALUES('$recipient_id','$message',1, '$sender_id');";
